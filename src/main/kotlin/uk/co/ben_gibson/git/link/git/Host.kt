...
sealed class Host(val id: UUID, val displayName: String, val icon: Icon, val baseUrls: Set<URL> = setOf())

class GitHub() : Host(
    UUID.fromString("72037fcc-cb9c-4c22-960a-ffe73fd5e229"),
    message("hosts.github.name"),
    AllIcons.Vcs.Vendors.Github,
    setOf(URL("https://github.com"))
)
...
class Chromium() : Host(
    ...
    Icons.CHROMIUM,
    setOf(URL("https://chromium.googlesource.com"))
)

class Custom(id: UUID, displayName: String, icon: Icon, baseUrls: Set<URL> = setOf()) : Host(id, displayName, icon, baseUrls)